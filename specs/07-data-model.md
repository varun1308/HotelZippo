...
- Service-role/admin only: `raw_reviews`, `raw_review_payloads`, `pipeline_runs`, `pipeline_run_hotels`, `curation_hotels`, `routestack_destinations`, `routestack_hotels`. (RLS enabled with **no** client policies — the service role bypasses RLS; authenticated/anon clients get zero rows.)
...
- **`routestack_destinations`** (migration `0011_routestack_id_cache.sql`, Phase 7 / booking) — caches the **stable** RouteStack destination handle per HotelZippo destination enum value, so a repeat booking skips the paid `search-destinations` call. Columns: `destination` (text PK, `check in ('Phuket','Hong Kong','Singapore','Maldives','Bali')`), `rs_destination_id` (text), `rs_destination_type` (text, nullable), `lat` / `long` (double precision), `resolved_at`.
