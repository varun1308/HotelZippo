...
### RouteStack payload debug capture (migration 0015 · `lib/booking/payload-log.ts` · flag `ROUTESTACK_DEBUG_PAYLOADS`)

Beyond the always-on OTEL **traces** (which carry only metadata — step / hotel_id / dates / success / code / latency — never payloads), an **opt-in** debug capture persists each RouteStack call's request/response for replay/inspection. **Off by default;** enabled only when `ROUTESTACK_DEBUG_PAYLOADS=1` (→ **13 · Environment**).

- **`raw_routestack_payloads`** (migration 0015) — **service-role-only** (RLS enabled, no policies), append-only debug log. Columns: `step, path, request, response, success, code, duration_ms, error, hotel_id, trace_id, created_at`. No FK on `hotel_id` (it's RouteStack's id string, table must survive without a `hotels` row). No dedup index (every call logged). Retention is **operator-managed** (no auto-TTL).
- **Always redacted before insert** — `redact()` masks the session `token`, `correlationId`, the deep-link payment `url` (it embeds the whole session), `Authorization`, and guest-PII fields (names/ages/contact) at any depth. The table inspects request/response **shape + business codes**, not secrets.
- **Seam:** `lib/booking/payload-log.ts` exposes a **`PayloadLog` interface** + `makeSupabasePayloadLog(serviceClient)`, injected as **`deps.debugLog`** on the orchestrator; `tracedCall` records every step (**success AND failure**). **Best-effort / fully isolated:** the fire-and-forget guards both a synchronous throw and a rejected promise, so a misbehaving log can never break a booking or surface an unhandled rejection. Absent the flag → `deps.debugLog` is undefined → zero capture, today's behaviour unchanged.
- **OTEL-logs half DEFERRED:** shipping redacted summaries to Dash0 via an OTEL **logs** pipeline is intentionally deferred (this is the DB half only) — `@vercel/otel` exposes no logs pipeline + breaks under tsx; revisit alongside the booking-confirmation-surfacing work.
...
6. **Completed-booking webhook/callback** — ⚠️ **SUPERSEDED (2026-06-28).** This originally read "Confirmed none." RouteStack **does** provide outbound HTTP webhooks (dashboard **Webhook Settings**, Live + Sandbox URLs + access key) that push the **order lifecycle** (`ORDER_CREATION_*` / `ORDER_PAYMENT_*` / `BOOKING_SUCCESS|FAILED|CANCELLED`), each carrying `orderid` + `billing_email`. HotelZippo now ingests these → see **[10d · Booking Webhooks](10d-booking-webhooks.md)** (inbound endpoint + `billing_email` correlation to a pending `booking_orders` row). `get-booking-info` / `cancel-booking` / `list-bookings` remain out of v1 scope.
