...
- **Phase:** 7 (final build phase) · **Status:** BUILT — Slices A (#23) + B (#24) merged; Slice C (capture script + env-gated sandbox smoke + fixture mapper) shipped. **Scope (founder, 2026-06-05):** booking is a **two-phase flow** with a **room/rate selection step** rendered as a **modal / bottom-sheet room picker** (not inline chat cards, not an accordion). Phase 1 returns the available rooms/rates; the user picks one; Phase 2 revalidates that specific rate → payment URL. **One live blocker remains (founder/RouteStack-admin, NOT code):** the sandbox account is not yet provisioned for hotel search — the partner-token HMAC→JWT exchange succeeds, but `search-destinations` returns *"Member token is required for this account … not anonymous … Use /mcp/auth/login or mark the account as anonymous"*, and `/mcp/auth/login` returns *"Partner credentials are required … contact the MCP administrator."* The founder must mark the sandbox account **anonymous** (server auto-generates a member token) or obtain **member credentials** for `/mcp/auth/login`. The env-gated smoke test asserts the JWT exchange and **skips the hotel assertions** gracefully until then; `npm run booking:capture` pins the real rooms/rates schema the moment the account is ready.
